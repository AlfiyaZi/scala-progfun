...
object Huffman {
 ...
 def chars(tree: CodeTree): List[Char] =  tree match{
   ...
   case Leaf(char,weight)=> List(char).toList
  }
  ...
  //def singleton(tree:List[CodeTree]):Boolean=tree match {
  //  case t :: Nil => true
  //  case _ => false
  //}
...
def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def tr(x: Int, p: (Char, List[Bit])): (Char, List[Bit]) = p match {
      case (c, bs) => (c, x :: bs)
    }
    a.map(tr(0, _)) ::: b.map(tr(1, _))
  }
