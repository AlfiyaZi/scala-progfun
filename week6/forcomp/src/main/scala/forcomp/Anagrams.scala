...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(e=>e).mapValues(x=>x.length).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =occurrences match {
    ...
    case (k,n) :: ys => combinations((k,n-1) :: ys) ++ combinations((k,n-1) :: ys).map(zs => zs.find(_._1 == k) match {
        case Some(_) => zs.map(_ match {
            case (d, o) => if (d == k) (d,o+1) else (d, o)
          })
        case None => (k,1) :: zs
      })
  }
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.map(x => y.find(_._1 == x._1) match {
